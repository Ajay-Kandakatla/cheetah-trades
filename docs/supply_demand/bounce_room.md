...
| step | rule | constant |
|---|---|---|
| overhead | supply bands with `hi >= print`, **plus demand bands with `lo > print`** (broken support = resistance, kind `broken_support`). A demand band that *contains* the print is support, never overhead. | same rule as `portfolio.supply_watch.overhead_bands`; re-stated here because the `portfolio` package cannot be imported on the py3.9 host — `test_overhead_rule_matches_portfolio_supply_watch_loaded_standalone` loads that file by path and compares every print |
