...
class FakeColl:
    ...
    def update_one(self, q, u, upsert=False):
        ...
        the result says whether THIS call inserted — the atomic dedupe claim
        (demand_alerts.claim_key) reads exactly that."""
    ...
    def find(self, q, projection=None):
        ...
        if "_id" in q:                                    # state: {"_id": {"$in": [...]}}
            ...
            return
        if "symbol" in q and "date" not in q:             # state by symbol (demand_alerts.recorded_today)
...
def test_minute_pass_says_bouncing_when_the_low_touched_the_band(monkeypatch):
    """Ajay 2026-09-08: the direction in writing. Same read as the 5-min pass."""
    sent = _capture(monkeypatch)
    store = {"AAA": _doc("AAA", [DEM, RES], 95.0)}
