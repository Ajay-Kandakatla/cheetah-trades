...
const PatternsPage                = lazyWithReload(() => import('./pages/PatternsPage').then(m => ({ default: m.PatternsPage })));
...
/* `/learn` (the flash-card bank) and `/chart-school` (its daily chart quiz)
   were DELETED 2026-09-20 — Ajay: "Delete Flashcards please". Both pages read
   the now-removed backend/flashcards/ API. `/learning` below is a DIFFERENT
   page (his study plan) and stays. */
...
const DeskPage                   = lazyWithReload(() => import('./pages/Desk').then(m => ({ default: m.Desk })));
...
export function App() {
  ...
  return (
    ...
    <div className="app">
      ...
      <MarketDayBorder />
      {/* Posture pill (explicit market read on every page, Ajay 2026-06-23)
          now lives INSIDE the nav bar's flex layout — see NavBar. It used to
          float here as position:fixed and overlapped the nav/content. */}
      <NavBar />
      <EngineStalledBanner />
      <BreakoutAlertBanner />
      ...
      <main className="main">
        <Suspense fallback={<PageLoader />}>
          <Routes>
            ...
            <Route path="/patterns"       element={<FeatureRoute feature="patterns"><PatternsPage /></FeatureRoute>} />
            ...
            <Route path="/desk"           element={<FeatureRoute feature="desk"><DeskPage /></FeatureRoute>} />
